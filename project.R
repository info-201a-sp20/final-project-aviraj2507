...
cali_tax_summary <- group_by(cali_tax_info, zipcode) %>%
  summarize(sum_of_agi = sum(A00100), sum_of_taxpayers = sum(N1), avg_tax_income = sum_of_agi / sum_of_taxpayers) %>%
...
cali_sat_income_by_zip <- left_join(cali_hs_satScores, cali_tax_summary, by = c("Zip" = "zipcode")) %>%
